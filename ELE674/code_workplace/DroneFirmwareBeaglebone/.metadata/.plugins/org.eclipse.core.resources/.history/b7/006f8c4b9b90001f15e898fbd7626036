...
/**/
		...
		timestamp = Sensor->RawData[DataIdx[0]].timestamp;
		...
		switch (Sensor->type) {
		  case ACCELEROMETRE :	LocData.Roll  = atan2( XYZ[0][X_AXE], sqrt(XYZ[0][Y_AXE]*XYZ[0][Y_AXE]+XYZ[0][Z_AXE]*XYZ[0][Z_AXE]));
			  	  	  	  	    LocData.Pitch = atan2(-XYZ[0][Y_AXE], sqrt(XYZ[0][X_AXE]*XYZ[0][X_AXE]+XYZ[0][Z_AXE]*XYZ[0][Z_AXE]));
//printf("Accel   : Roll = %10.5lf  Pitch = %10.5lf\n", LocData.Roll, LocData.Pitch);
			  	  	  	  	  	break;
		  case GYROSCOPE :		LocSpeed.Roll  =  (XYZ[0][X_AXE]);
		  	  	  	  	  	  	LocSpeed.Pitch = -(XYZ[0][Y_AXE]);
		  	  	  	  	  	  	LocSpeed.Yaw   = -(XYZ[0][Z_AXE]);
//printf("Gyro    : Roll = %10.5lf  Pitch = %10.5lf  Yaw = %10.5lf\n", LocSpeed.Roll, LocSpeed.Pitch, LocSpeed.Yaw);
	  	  	  	  				break;
		  case SONAR :			LocData.Elevation  = (XYZ[0][X_AXE]);
		  	  	  	  	  	  	LocSpeed.Elevation = ((XYZ[0][X_AXE])-(XYZ[1][X_AXE]))/Ts;
//printf("Sonar   : Elevation = %10.5lf  ElevationSpeed = %10.5lf\n", LocData.Elevation, LocSpeed.Elevation);
	  	  	  	  				break;
		  case BAROMETRE :		break;
		  case MAGNETOMETRE :	My =  XYZ[0][Y_AXE]*cos(LocData.Roll) - XYZ[0][Z_AXE]*sin(LocData.Roll);
								Mx =  XYZ[0][X_AXE]*cos(LocData.Pitch) + XYZ[0][Y_AXE]*sin(LocData.Roll)*sin(LocData.Pitch) + XYZ[0][Z_AXE]*cos(LocData.Roll)*sin(LocData.Pitch);
								LocData.Yaw = atan2(My,Mx);
//printf("Magneto : Yaw = %10.5lf\n", LocData.Yaw);
  								break;
		}
/**/
		pthread_spin_lock(&(AttitudeMesure->AttitudeLock));
		...
		AttitudeMesure->timestamp = timestamp;
		pthread_spin_unlock(&(AttitudeMesure->AttitudeLock));
